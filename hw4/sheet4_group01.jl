...
md"""
# Sheet 4: Matrix eigenvalue bounds
*(not to be handed in)*
"""
...
md"""
## Exercise 1
We are in the setting of Theorem 3.6, i.e.:

Let $\tilde{v}$ be an approximate eigenvector to $A$, $\|\tilde{v}\| = 1$,
$\tilde{λ} = \langle \tilde{v}, A \tilde{v} \rangle$ and $r = A \tilde{v} -\tilde{λ} \tilde{v}$. Let $\lambda$ be the eigenvalue closest to $\tilde{λ}$, let $\delta = \min_i \{ |\lambda_i - \tilde{λ}|, \lambda_i \neq \lambda \}$ be the distance from the spectrum and let $v$ be an eigenvector of $A$ associated with $λ$.

Prove the improved error
```math
\sin θ(\tilde{v}, v) ≤ \sqrt{\frac{\|r\|_2^2 - ε^2}{\delta^2 - \varepsilon^2} },
```
where $ε = |\lambda - \tilde{\lambda}|$.
...
# ╔═╡ ba186369-0459-4027-b4ff-f50a2e2f3555
md"""
### Final plot
This section compiles a nice plot of the errors and Gerschgorin disks as we go along, see below for the tasks.
"""

# ╔═╡ cc046b49-b379-4006-b45d-d09d2ade774a
md"""
...
First we want to add a plot of the Gerschgorin disks to the plot above. Add code to compute the centres:
"""
...
md"""
### Approximate eigenpairs and residuals
"""

# ╔═╡ e2155c0c-787f-400f-afd5-1f3aac2ab30d
md"""
Clearly $M$ is nearly a diagonal matrix for most choices of the parameters. We thus take the diagonal of $M$ as our estimate for the eigenvalues:
"""

# ╔═╡ dad89f86-0e7a-41de-9de8-4df469e65179
computed_eigenvalues = diag(M)

# ╔═╡ 23c5ed2b-7b4d-427f-be1a-2cf4fa21dfbb
md"""A natural question is: How accurate is this estimate?

For comparison we compute the exact eigenvalues. Add code to do so:
"""
...
md"""
Apart from this computation we will assume from here on that we know nothing about the exact spectrum and use the techniques developed in the lecture to estimate the error of `computed_eigenvalues`.
"""

# ╔═╡ 0fea8e6a-549e-4175-8337-a21783a82333
md"""
Both Bauer-Fike and Kato-Temple need the **residuals**. Thus we first compute these.
...
With the residuals in place we can proceed to compute the error estimate following the **Bauer-Fike theorem**.

The following code should produce a vector of error estimates, one for each eigenvalue
"""
...
if ismissing(error_Bauer_Fike)
	still_missing()
elseif !type_eq(error_Bauer_Fike, Vector{Float64})
	wrong_type(:error_Bauer_Fike, Vector{Float64})
end
...
```math
δ_1 = |\tilde{λ}_1 - λ_2| ≥  |\tilde{λ}_1 - \tilde{λ}_2| - \|r_2\|.
```
For the second and higher eigenpairs we need to keep in mind that there are two neighbouring eigenvalues and we need to take the *smallest* distance, thus we have
```math
\begin{aligned}
δ_{2l} &= |\tilde{λ}_2 - λ_1| ≥ |\tilde{λ}_2 - \tilde{λ}_1| - \|r_1\| \\
δ_{2r} &= |\tilde{λ}_2 - λ_3| ≥ |\tilde{λ}_2 - \tilde{λ}_3| - \|r_3\| \\
δ_2    &= \min(δ_{2l}, δ_{2r})
\end{aligned}
```
An additional complication is that as the residuals $\|r_i\|$ get too large and $\delta_2$ becomes negative. The interpretion in this case is that it is no longer guaranteed that our numerical scheme provides the correct ordering of the approximate eigenvalues $\tilde{λ}_i$ versus $λ_i$. The Kato-Temple bound in this case is not applicable, which we fix by explicitly setting $δ_i$ to zero (and thus causing an infinite Kato-Temple bound). We thus need an additional step where we set
```math
δ_i \leftarrow \max(0, δ_i)  \qquad \forall i.
```
...
if ismissing(δ)
	still_missing()
elseif !type_eq(δ, Vector{Float64})
	wrong_type(:δ, Vector{Float64})
end
...
begin
	...
	if !ismissing(exact_eigenvalues)
	...
	for i in 1:size(M, 1)
		if (!ismissing(geschgorin_centres) && !ismissing(geschgorin_radii)
		    && show_geschgorin)
...
if ismissing(error_Kato_Temple)
	still_missing()
elseif !type_eq(error_Kato_Temple, Vector{Float64})
	wrong_type(:error_Kato_Temple, Vector{Float64})
end
