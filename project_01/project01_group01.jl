...
begin
	...
	for Nb in collect(500:200:2600)
		ε_QM = solve_discretised(v_chain, Nb, fixed_a; n_ep=3).λ[1]
		ε_CL = solve_discretised(v_atom, Nb, fixed_a; n_ep=3).λ[1]
...
begin
	...
	for a in collect(4:1:10)
		ε_QM = solve_discretised(v_chain, found_Nb, a; n_ep=3).λ[1]
		ε_CL = solve_discretised(v_atom, found_Nb, a; n_ep=3).λ[1]
...
begin
	ε_QM = solve_discretised(v_chain, found_Nb, found_a; n_ep=3).λ[1]
	ε_CL = solve_discretised(v_atom, found_Nb, found_a; n_ep=3).λ[1]
	println(ε_CL - ε_QM)
end
...
begin
	result_QM = solve_discretised(v_chain, found_Nb, found_a; n_ep=3)
	residual_norms_QM = residual_norms_interval(v_chain, result_QM.λ, result_QM.X, found_Nb, found_a)
	upper_bound_QM = get_upper_bound(result_QM, residual_norms_QM)
end

# ╔═╡ 9ae0dccf-9282-4813-9ab2-9c63c901039e
begin
	result_CL = solve_discretised(v_atom, found_Nb, found_a; n_ep=3)
	residual_norms_CL = residual_norms_interval(v_atom, result_CL.λ, result_CL.X, found_Nb, found_a)
...
begin
	M = 1
	QM = solve_discretised(v_extended_chain(M), found_Nb, found_a; n_ep=3).λ[1]
	CL = solve_discretised(v_atom, found_Nb, found_a; n_ep=3).λ[1]

	tun_energy = CL - QM
end

# ╔═╡ 6d765847-a4d0-402e-92a7-148577f2cefb
CL

# ╔═╡ 7ddcbf88-0a83-4230-9a64-7418e799578f
QM
...
# ╔═╡ c63e2d4e-1747-410d-8f13-53a8320846b5
solve_discretised_QM(v_extended_chain(1), found_Nb, found_a; n_ep=3).λ[1]
...
begin
	...
	m = 1
	local a = 2 * m + 2
	local Nb = round(Int64, 2 * a / h_found) + 1
	...
	for m in 2:1:30
		a = 2 * m + 2
		Nb = round(Int64, 2 * a / h_found) + 1
				
		noise = 0.001 * randn(Float64, size(X_prev))
		X_prev = X_prev + noise

		n = round(Int64, 0.5 * (Nb - size(X_prev)[1]))
		padding = zeros(Float64, n, 3)
		X_prev = vcat(padding, X_prev, padding)

		res = solve_discretised_QM(v_extended_chain(m), Nb, a; X_32=Float32.(X_prev), n_ep=3)
		X_prev = res.X
		QM = res.λ[1]
...
md"""
By changing the value of $\widetilde{M}$ we can see that tunneling energy is converging, in our case to -0.25.
