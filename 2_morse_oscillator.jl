...
In this exercise we want to explore the different error contributions based on the [Morse oscillator](https://en.wikipedia.org/wiki/Morse_potential). The Morse oscillator is a simple quantum-mechanical model for a chemical bond. Even though nowadays more accurate models exist, it is still used due to its simplicity. One remarkable feature of the model is that it is able to produce a realistic description of chemical bonding. At the same time the associated Schrödinger equation still has an analytic solution --- even though we will not attempt this computation here: See [wikipedia](https://en.wikipedia.org/wiki/Morse_potential) if you are curious.
...
md"""for which the potential looks as such"""

# ╔═╡ 093f0ea9-f570-4307-9235-24cfb576a57c
Vm(x) = D * (1 - exp( -ω / sqrt(2D) * (x - x0) ) )^2

# ╔═╡ 506def7a-19b9-4fe8-b836-524f8a13013f
plot(Vm, xlims=(-3, 10), ylims=(-1, 20), label="Morse potential")

# ╔═╡ 061ff8be-a2e3-4c8d-a8e4-c76bd2df1d15
md"""
Following our discussion about the Schrödinger equation in the lectures, we are interested in the eigenvalues and eigenfunctions of this Hamiltonian, i.e. the $(E^M_n, \varphi^M_n) \in \mathbb{R} \times L^2(\mathbb{R})$ pairs satisfying
```math
H^M \varphi^M_n = E^M_n \varphi^M_n.
```
As mentioned above, this system can actually be solved *analytically* with its eigenvalues given by
```math
E^M_n = \omega \left(n + \frac12 \right) - \omega \chi \left(n + \frac12\right)^2
\qquad n = 0, 1, 2, \ldots, \lfloor\tfrac{2D}{\omega} - \tfrac{1}{2} \rfloor
```
where
```math
χ = \frac{ω}{4D}
...
While the Morse oscillator provides already a relatively simple model for a chemical bond, a yet even simpler model is often sufficent: The [Harmonic oscillator](https://en.wikipedia.org/wiki/Quantum_harmonic_oscillator). Its potential simply takes the form
```math
V^H(x) = \frac12 ω^2 (x - x_0)^2.
```
Again we are able to find analytical eigenpairs $(E^H_n, \varphi^H_n)$ of the Harmonic oscillator Hamiltonian
```math
H^H = - \frac12 \Delta + V^H = - \frac12 \Delta + \frac12 ω^2 (x - x_0)^2,
```
where in particular
```math
E^H_n = \omega \left(n + \frac12\right)\qquad\text{for $n = 0, 1, 2, \ldots$}
```
...
```math
\tilde{\varphi}^H_0(x) = \exp\left(- \tfrac{1}{2}\omega (x - x_0)^2 \right)
```
...
We perform an $N$-point finite-difference approximation within $\Omega$, i.e. we split the domain into $N$ intervals of length $h = \frac{2a}{N-1}$. Recall that for this setup the finite-difference Laplacian is given by
"""
...
hint(
md"""
You can use the `range` function to generate a vector of grid points.
"""
)
...
hint(
md"""
To extract the first column of a matrix `A` as a vector, you can use `A[:, 1]`.
"""
)
...
md"""
**(c)** Use the expressions for the exact ground state energy and ground state eigenfunction $\varphi_0$ from *Exercise 1* to verify that the eigenvalue is converged to $10^{-4}$ and the $L^2(\mathbb{R})$-error of the eigenvector is also around $10^{-4}$.

*Hint:* Note, that `eigen` does indeed return the finite difference approximation $(f(x_1), \ldots, f(x_n))^T$ to the eigenfunction $f$, where $\{x_i\}_{i=1}^N$ are the points of your finite differences grid. However, since it normalises the vectors in $\mathbb{R}^N$ using the Euclidean norm, the resulting approximate function $f$ is *not* appropriately $L^2(\mathbb{R})$-normalised. To see this, consider the finite differences approximation to the integral 
```math
\int_\Omega f(x) dx,
```
...
hint(
md"""
It might be useful to write two helper functions:
1. A function `discretized_l2_norm(f, a)` to approximate the $L^2(\mathbb R)$-norm of a vector `f` of length $N$ representing function discretized to the equispaced grid of $N$ points on the interval $[-a,a]$.
2. A function `discretized_l2_error(f1, f2, a)` to approximate the $L^2(\mathbb R)$-error between two discretized functions, also using 1.
"""
)
...
function inverse_power_method(A; u=randn(eltype(A), size(A, 2)),
                              tol=1e-6, maxiter=500)
	norm_Δu = NaN
	Afac = factorize(A)  # Factorise A to make A \ x more economical
	for i in 1:maxiter
		u_prev = u
		u = Afac \ u
		normalize!(u)
	...
	μ = dot(u, A, u)
	norm_Δu ≥ tol && @warn "Inverse power not converged $norm_Δx"
	(; μ, u)
end
...
begin
	fd_Hh_64 = - 0.5 * fd_laplacian(N, a; T=Float64) + 0.5 * ω^2 * Diagonal(grid_points.^2)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "e6abf62d24e0c259ba7942121920191b88fff29b"
