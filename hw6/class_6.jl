...
md"""
# 5 Overview of diagonalisation algorithms
...
A x_i = \lambda_i x_i
```
where we impose the ordering $\lambda_1 \leq \lambda_2 \leq \cdots \leq \lambda_N$.
"""
...
We already discussed the **power method** in previous lectures and exercises as a way to obtain the eigenvector $x_N$ corresponding to the largest eigenvalue. Via the Rayleigh quotient the corresponding eigenvalue $\lambda_N$ then becomes accessible as well.

For completeness our algorithmic receipe to obtain the eigenvector to tolerance $\varepsilon$ was:
...
md"""
From playing with the slider we notice that when the largest two eigenvalues get too close, the convergence deteriorates. This can be understood using the following theorem:

**Theorem 5.1** Assume that $\lambda_N \neq \lambda_{N-1}$, i.e. that the largest eigenvalue is simple. Than the initial vector $x^{(0)}$ has either no overlap with the eigenvector $x_N$ or the sequence $x^{(j)}$ generated by the power method converges to $x_N$.

This is a well-known result for which proofs can be found e.g. in the book by Saad or [on wikipedia](https://en.wikipedia.org/wiki/Power_iteration). Key outcome of the proof is that the convergence rate depends on the ratio
```math
\frac{|λ_{N-1}|}{|λ_{N}|}
```
thus deteriorates as $λ_{N-1}$ and $λ_N$ are too close, e.g. if $λ_N$ is a multiple eigenvalue.
"""
...
md"""
Part 1 of this theorem is the justification for using the inverse power method to approximate the smallest eigenpair of $A$ as done already in one of the previous exercises. Part 3 of this theorem, called **shift and invert** provides us with a receipe to compute an arbitrary isolated eigenvalue.
...
md"""Suppose we don't know the exact eigenvalues, but we know that the eigenpair $(λ_i, x_i)$ we are interested in has an eigenvalue around $0.4$. Then we set $σ = 0.4$ and employ the matrix $(A - σ I)^{-1}$ in the `power_method`, which as per our previous discussion will converge to the eigenvalue $\frac{1}{λ_i - σ}$, which is largest. Now this is clearly the case for the $λ_i$ closest to $σ$.
From Theorem 5.2 we additionally know that the eigenvector of $(A - σ I)^{-1}$ associated to $\frac{1}{λ_i - σ}$ is exactly the eigenvector of $A$ associated to $λ_i$. Therefore we have computed the *exact* eigenpair $(λ_i, x_i)$ based on the approximate eigenvalue $σ = 0.4$.
...
let
	...
	# Compute eigenvalue using Reighlay quotient of original matrix
	λ_reighlay = dot(x, Ashift, x)

	println()
	println("Power method converged to eigenvalue: $λ")
	println("Exact value for 1/(λ₂ - σ) is:        $(1/(λ₂ - σ))")
	println("Computed eigenvalue of Ashift:        $λ_reighlay")
...
\end{aligned}
```
This function is clearly minimial for $z = 0$, which concludes the proof. 
...
md"""Notice, how the the convergence in the last few steps is *very fast*, such that once a reasonably good quality for the Rayleigh quotient as the initial starting vector has been reached, multiple orders of magnitude in the residual norm are gained in each step.
...
Asubspace = Diagonal([150., 100., 100. + 10^logsubδ, 50., 40., 30.])
...
```
An orthogonal projection technique no seeks approximate eigenpairs $(\tilde{\lambda}_i, \tilde{x}_i)$ with $\tilde{\lambda}_i \in \mathbb{R}$, $\tilde{x}_i \in \mathcal{S}$, such that the following **Galerkin condition**
```math
...
```
Suppose now that $\{v_1, v_2, \ldots, v_n\}$ is an orthonormal basis for $\mathcal{S}$ and denote by $V = (v_1, v_2, \ldots, v_n) \in \mathbb{C}^{N\times m}$ the matrix with these basis vectors as columns. Expanding $\tilde{x}_i$ in this basis, i.e. introducing a vector $\tilde{y}_i \in \mathbb{C}^m$ with
```math
...
> 3. Use `eigen` to compute the eigenpairs $(\tilde{λ}_i, \tilde{y}_i)$ of $A_V$ and select the $k$ desired ones.
...
Note, that the eigenpairs $(\tilde{λ}_i, \tilde{y}_i)$ of the subspace matrix $A_V$ are usually also called **Ritz pair** with $\tilde{λ}_i$ called **Ritz values** and $\tilde{y}_i$ a **Ritz vector**. 

One way to put this into practice is to employ the ideas of the `subspace_iteraiton` discussed above. Recall that this method generates in each iteration a set of orthogonal vectors `U` as guesses for the eigenpairs to be determined. These vectors span a subspace, which can be employed in a Rayleigh-Ritz procedure, leading to the projected subspace iterations:
"""
...
**Proposition 5.5.** The $i$-th approximate eigenvalue (counted from small to large including mutliplicities) of a Hermitian matrix $A$ obtained from an orthogonal projection method to a subspace $\mathcal{S}$ satisfies
```math
...
λ_i \leq \tilde{λ}_i
```
holds, i.e. eigenvalues are **approxmiated from above** in orthogonal projection methods.
...
md"Convergence is thus great if a good preconditioner is chocen. However for no preconditioner we obtain:"
...
If one considered preconditoned gradient ascent one could
...
What preconditioner should be chosen is often highly problem dependent. However, there are a number of standard choices, which can often be employed blindly to speed up the calculations. Popular are in particular incomplete factorisations (e.g. Choleski) or lower-precision version of the original problem. A detailed discussion is out of the scope of these notes. Details can be found, for example, in book of Saad.
...
The final aspect we want to briefly address is the question how to chose the step size $α$. The best step size can be found by a line search,
...
```
This turns out to improve the overall observed convergence rates considerably. This modification has been suggested following a more detailed investigation of minimisation approaches for eigenvalue problems, in particular a comparision with more sophisticated minimisation algorithms such as the conjugate gradient (CG) approach, which is out of the scope of this lecture.
...
These two modifcations with our previous discussion lead to the LOPCG (locally optimal preconditioned conjugate gradient) algorithm:
"""
...
function lopcg(A; x=randn(eltype(A), size(A, 2)), ortho=ortho_qr,
	...
	for i in 1:maxiter
		...
		new_x = Z * y  # x^{(i)} is the eigenvector of the Reighlay-Ritz,
...
md"""
The generalisation to an algorithm capable of computing multiple eigenpairs is straightforward, leading to LOBPCG (locally optimal *block* preconditioned conjugate gradient) is only book-keeping:
"""
...
md"""
Note, that this simple implementation of LOBPCG is still not numerically stable and considerable issues remain. In practice one should thus always employ one of the well-tested implementation instandard numerical libraries.
...
We set up a test problem, where we can tune both the size of the matrix as well as
the gaps between the the first eigenvalues. We then try multiple methods to obtain the **smallest** one or two eigenpairs:
"""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "cbfe3587380efb77c413f83c962bf5e44138a36c"
